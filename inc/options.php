...
class HeartySliderSettingsPage {
    ...
    ?>
        ...
        <div class="hearty-admin">
            <h1 class="hearty-admin-title">Hearty Slider | Settings</h1>
            <h5 class="hearty-admin-description"><a href="https://www.heartyplugins.com/wordpress-plugins/free-plugins/product/hearty-slider" target="_blank">See details and documentation</a></h5>
            <form method="post" action="options.php" class="hearty-admin-form" autocomplete="off">
                <?php
                    settings_fields('heartyslider_options_group');
                    do_settings_sections('heartyslider-setting-admin');
                    submit_button();
                ?>
            </form>
        </div>
    ...
    public function page_init() {
        ...
        $this->options = get_option('heartyslider_options');
        $this->number_of_settings_instances = 1;
        $this->number_of_content_items = 10;

        register_setting(
            'heartyslider_options_group',
            'heartyslider_options',
            array($this, 'sanitize')
        );

        $this->generate_sections();

        HeartySliderParams::generate_fields($this, $this->number_of_settings_instances, $this->number_of_content_items);
	...
	public function generate_sections() {
		...
		for ($i = 1; $i <= $this->number_of_content_items; $i++) {
			...
			add_settings_section(
				'heartyslider_content_options_section_'.$i,
    ...
    public function fields_callback($args) {
        ...
        if ($args['type'] == "text") {
        	...
        	$output .= '<input type="text" class="'.$args["class"].'" id="'.$args["id"].'" name="heartyslider_options['.$args["id"].']" value="'.(isset($this->options[$args["id"]]) ? esc_attr($this->options[$args["id"]]) : $args["default"]).'" />';

        } else if ($args['type'] == "textarea") {

        	$output .= '<textarea id="'.$args["id"].'" name="heartyslider_options['.$args["id"].']">'.(isset($this->options[$args["id"]]) ? esc_attr($this->options[$args["id"]]) : $args["default"]).'</textarea>';

        } else if ($args['type'] == "select") {

        	$output .= '<select id="'.$args["id"].'" name="heartyslider_options['.$args["id"].']">';
        	...
        	$svalue = isset($this->options[$args["id"]]) ? esc_attr($this->options[$args["id"]]) : $args["default"];
        	$output .= '<input type="hidden" id="'.$args["id"].'" name="heartyslider_options['.$args["id"].']" value="'.$svalue.'" />';
    ...
    public function media_callback($args) {
        ...
        if ($args['type'] == "text") {
			...
			$output .= '<input id="'.$args["id"].'" type="text" name="heartyslider_options['.$args["id"].']" value="'.(isset($this->options[$args["id"]]) ? esc_attr($this->options[$args["id"]]) : $args["default"]).'" />';
